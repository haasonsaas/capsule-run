use capsule_run::api::{ExecutionRequest, ResourceLimits, IsolationConfig};
...
async fn test_basic_execution() {
    ...
    match response.status {
        ...
        _ => {
            ...
            println!("Test failed due to sandbox restrictions: {:?}", response.error);
        }
    }
}
...
async fn test_environment_variables() {
    ...
    let request = ExecutionRequest {
        command: vec!["sh".to_string(), "-c".to_string(), "echo $TEST_VAR".to_string()],
        environment,
        timeout_ms: 5000,
        resources: ResourceLimits::default(),
        isolation: IsolationConfig::default(),
